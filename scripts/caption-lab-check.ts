...
// 2. SRT → linhas
const lines = parseSrt(DEMO_SRT);
check("SRT gera 3 linhas", lines.length === 3, `${lines.length}`);
check("primeira linha começa em 0", lines[0].start === 0);
check("última palavra fecha no fim do bloco", lines[0].words.at(-1)!.end === 2.4);
...
// 8. ASS por preset e por formato
...
const tiny = buildAss(lines, { presetId: "hormozi", videoWidth: 1080, videoHeight: 1920, fontScale: 0.35 });
const big = buildAss(lines, { presetId: "hormozi", videoWidth: 1080, videoHeight: 1920, fontScale: 1.8 });
...
// 10. Posições
check("center usa alignment 5", buildAss(lines, { presetId: "clean", videoWidth: 1080, videoHeight: 1920, position: "center" }).style.align === 5);
check("top usa alignment 8", buildAss(lines, { presetId: "clean", videoWidth: 1080, videoHeight: 1920, position: "top" }).style.align === 8);
...
const kara = buildAss(lines, { presetId: "karaoke", videoWidth: 1080, videoHeight: 1920 });
check("karaokê emite tags \\kf", kara.ass.includes("\\kf"));
const typed = buildAss(lines, { presetId: "hormozi", videoWidth: 1080, videoHeight: 1920, animation: "typewriter" });
check("typewriter esconde o resto com \\alpha&HFF&", typed.ass.includes("\\alpha&HFF&"));
const pop = buildAss(lines, { presetId: "hormozi", videoWidth: 1080, videoHeight: 1920, animation: "pop" });
