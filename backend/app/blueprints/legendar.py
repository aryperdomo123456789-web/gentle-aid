"""Ferramenta 3 — Legendagem dinâmica."""
...
STYLES = {"viral", "karaoke", "clean", "neon"}
...
def run_job():
    ...
    job = jobs.create_job("legendar", meta={"style": style, "position": position})
    try:
    ...
    jobs.submit(job["job_id"], lambda jid: _work(jid, src, srt_text, style, position))
    return jsonify(job), 202


def _work(job_id: str, src: Path, srt_text: str, style: str, position: str) -> None:
    jobs.update(job_id, md5_before=media.md5(src), progress=20)

    srt_path = output_path("legendar", job_id, ".srt")
    if srt_text.strip():
        content = srt_text if "-->" in srt_text else _plain_to_srt(srt_text, media.probe_duration(src))
    else:
        jobs.log(job_id, "Nenhuma transcrição enviada — gerando cartela padrão.")
        content = _plain_to_srt("Legenda automática", media.probe_duration(src))
    srt_path.write_text(content, encoding="utf-8")

    dst = output_path("legendar", job_id, "_legendado.mp4")
    jobs.log(job_id, f"Queimando legendas (estilo {style}, posição {position})")
    media.burn_subtitles(src, srt_path, dst, job_id=job_id, style=style, position=position)
    ...
    jobs.update(
        ...
        status="done",
        progress=100,
        message="Vídeo legendado gerado.",
...
def _plain_to_srt(text: str, duration: float) -> str:
    ...
    for i, chunk in enumerate(chunks):
        ...
        lines.append(f"{stamp(i * step)} --> {stamp((i + 1) * step)}")
        lines.append(chunk)
        lines.append("")
    return "\n".join(lines)
